<template>
  ...
  >
    ...
    <Column header="Подъезды" style="width: 5%" :pt="columnStyle">
      <template #body="slotProps">
        <div class="flex flex-col justify-center items-center">
          <div
            v-html="slotProps.data.entrances"
    ...
    <Column field="quantity" header="Этажи" style="width: 5%" :pt="columnStyle">
      <template #body="slotProps">
        <p class="text-center text-grey-900">{{ slotProps.data.floors }}</p>
      </template>
    </Column>
    <Column
      field="quantity"
    ...
    >
      <template #body>
        <div class="w-full flex justify-center">
          <img :src="require('@/shared/assets/images/said.png')" alt="" />
        </div>
      </template>
    </Column>
      ...
      <template #body="slotProps">
        <div class="flex 2xl:flex-row xl:flex-col items-center">
          ...
          <Button
            ...
            @click="showTemplate($event, slotProps.index)"
          >
            <svg
              width="19"
              height="18"
...
const columnStyle = ref<PTOptions<ColumnPassThroughOptions>>({
  headerCell: {
    class: [
...
const floors = ref([
  {
    ...
    name: "f65218f1bae5442cba9ab1d61c410c0f.png",
    entrances: "Общая планировка",
    floors: "Этажи",
    sideWorld: "",
  },
]);
const selectedFloors = ref([]);
const showTemplate = (event: MouseEvent, index: number) => {
  confirm.require({
...
const op = ref();
const hoverImageHandler = (event: MouseEvent) => {
  op.value.show(event);
};
const unhoverImageHandler = (event: MouseEvent) => {
  op.value.hide(event);
};
</script>

<template>
  <div class="statusStep">
    ...
    <div class="statusStep__desc mb-7.5">
      Сопоставьте статусы из файла со статусами в Profitbase
    </div>
    <div class="grid grid-cols-2 gap-x-16 max-w-[1200px] mb-12">
      <div class="statusFromFile flex flex-col gap-y-5">
        <div class="title font-bold">Статусы, найденные в файле</div>
        <InputText
          disabled
          v-for="(status, index) in statusFromFile"
          :key="index"
          :value="status.name"
        />
      </div>
      <div class="statusFromFile flex flex-col gap-y-5">
        ...
        <Dropdown
          v-for="(status, index) in statusFromFile"
          :key="index"
          v-model="statusFromSystem[index]"
          :options="statusFromFile"
          option-label="name"
          :pt="{
            root: { class: '!py-0' },
            input: { class: '!p-0 !flex items-center' },
          }"
        >
    ...
    <div class="statusStep__desc mb-7.5">
      Сопоставьте типы помещений из файла с типами в Profitbase
    </div>
    <div class="grid grid-cols-2 gap-x-16 max-w-[1200px] mb-7.5">
      <div class="statusFromFile flex flex-col gap-y-5">
        ...
        <InputText
          disabled
          v-for="(status, index) in typeRoomFromFile"
          :key="index"
          :value="status.name"
        />
      </div>
      <div class="statusFromFile flex flex-col gap-y-5">
        ...
        <Dropdown
          v-for="(status, index) in typeRoomFromFile"
          :key="index"
          v-model="typeRoomFromSystem[index]"
          :options="typeRoomFromFile"
          option-label="name"
          :pt="{
            root: { class: '!py-0' },
            input: { class: '!p-0 !flex items-center' },
...
const statusFromFile = ref<{ id: string; name: string }[]>([
  { id: "booking", name: "Бронь" },
  { id: "sold", name: "Продано" },
  { id: "reserve", name: "Резерв" },
  { id: "freely", name: "Свободно" },
]);

const statusFromSystem = ref<{ id: string; name: string }[]>([]);

const typeRoomFromFile = ref<{ id: string; name: string }[]>([
  { id: "flat", name: "Квартира" },
  { id: "office", name: "Офис" },
  { id: "Storeroom", name: "Кладовая" },
]);

const typeRoomFromSystem = ref<{ id: string; name: string }[]>([]);
</script>
